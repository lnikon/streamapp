...
bool MediaController::isAuthorizationOk(const oatpp::String &authHeader, const AuthDataStorage& authDataStorage)
{
    if(authHeader.get() == nullptr)
    {
        return false;
    }

    const auto authValue = authHeader->std_str();
    const bool authValueExists = authDataStorage.isAuthValueExists(authValue);
    return authValueExists;
}
